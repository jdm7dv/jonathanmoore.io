...
$(document).ready(function() {
    ...
    if (stopProcessing == true) return;
    $(".errorSection").hide();
    $(".processingCss").show();
    $(".processingCss").attr("disabled", "disabled");
    $(".processingCss").text(postComment);
    $(".buttonCss").hide();
    var commentCss = $(".commentsCss");
    ...
    $(".SocialCss").click(function() {

        var t = $(this).attr('targetUrl');
    ...
    $(".icon_Delete").click(function() {
        ...
        if (!confirm("Do you want to delete this comment ?"))
            return false;
        $(this).attr('src', deleteProcessgif);
        ...
        cmt.ShortId = shortid;
        cmt.Locale = locale;
        cmt.ProductVersion = productversion;
        cmt.locationHref = window.location.href;


        var d = JSON.stringify(cmt);
        var jsonData = '{"cmt":' + d + '}'
        ...
        function DeleteSucceded(result) {
            prntId.hide();
            commentsCnt = commentsCnt - 1;

            var counttxt = "<b> " + commentsTopLeft + "</b> (" + commentsCnt.toString() + ")";

            $(".cmtslblClass").html(counttxt);

        }

        function DeleteFailed(result) {
            $("#errorLbl").text(passporterror);
            $(".errorSection").show();
        }
    ...
    function InserComment(commnt) {
        ...
        cmt.authorFirstName = authorFirstName;
        cmt.authorLastName = authorLastName;


        $(".processingCss").text(processingTxt);
    ...
    $(".buttonCss").click(function() {
        ...
        processing = true;
        rawtxt = $(".commentsCss").val();


        if (rawtxt.trim().length == 0) {
            $("#errorLbl").text(commentEmptyError);
            $(".errorSection").show();
            $(".buttonCss").show();
            $(".processingCss").hide();
            processing = false;
        ...
        rawtxt = rawtxt.replace(new RegExp('"', ""), "%22");
    ...
    $(".buttonCssDown").click(function() {

        rawtxt = $(".commentsCssDown").val();
        if (rawtxt.trim().length == 0) {
            $("#errorLbl").text(commentEmptyError);
            $(".errorSection").show();
            return false;
    ...
    function AjaxSucceded(result) {
        ...
        $(".usrComments").append(resultRow.rawHtml);

        var objDiv = $(".commentsDisplay");
...
function reportAbuse(target) {   
    window.open(target, 'SiteFeedback', 'width=670,resizable=no,height=700,toolbar=no,location=no,scrollbars=yes,directories=no,status=no,menubar=no'); 
    return false;
}

var interval;
var txtBox;
$(".commentsCss").focus(function() {
    txtBox = $(this);
    interval = window.setInterval(substringset, 1000);
});



$(".commentsCss").blur(function() {
    txtBox = $(this);
    clearInterval(interval);
    substringset();
});

function substringset() {
    ...
    if (txtBox.val().length > 0) {
        ...
        $(".processingCss").show();
        $(".processingCss").val(postComment);
        $(".buttonCss").hide();
    
    }
    if ($(".commentsCss").val().length >= 3100) {

        var t = $(".commentsCss").val().substr(0, 3100);
...
if (!this.JSON) { JSON = {}; }
(function() {
    function f(n) { return n < 10 ? '0' + n : n; } if
    (typeof Date.prototype.toJSON !== 'function') {
        Date.prototype.toJSON = function(key) {
            return this.getUTCFullYear() + '-' + f(this.getUTCMonth() + 1) + '-' + f(this.getUTCDate())
...
function doPaste(obj) {
    maxLength = obj.getAttribute("maxlength");
    if (maxLength) {
        if ((window.event) && (navigator.userAgent.toLowerCase().indexOf("safari") + 1 == 0)) { 
...
var IsObjectRated = false;
$(document).ready(function() {

    try {
        obj = document.getElementById(RatingObjId);
        IsObjectRated = obj.Rated;
    }
    catch (Error) {
...
$(".ratingsOnlyStar").hover(
function () {

    var israted = $(".ratingsOnlyClass").attr("isRated");
    if (israted == "true") {
        $(".ratingsOnlyClass").attr("title", RatedTooltipText);
        return;
    }
    else {
        $(".ratingsOnlyClass").attr("title", RatingTooltipText);
    }
    ...
    while (i < cnt) {
        i++;
        $("span[counter='" + i + "']").removeClass("emptyRatingStar");
        $("span[counter='" + i + "']").addClass("filledRatingStar");
    }
    while (i < 5) {
        i++;
        $("span[counter='" + i + "']").removeClass("filledRatingStar");
        $("span[counter='" + i + "']").addClass("emptyRatingStar");
    }
    $(".ratingsOnlyClass").data("rating", i);
},
function () {
    var cnt = $(this).attr("counter");
    var israted = $(".ratingsOnlyClass").attr("isRated");
...
$('.tableCellRateControlCss').bind('mouseleave', function () {
    var i = 0;
    while (i < parseInt(curRating)) {
        i++;
        $("span[counter='" + i + "']").removeClass("emptyRatingStar");
        $("span[counter='" + i + "']").addClass("filledRatingStar");
    }
...
$(".ratingsOnlyClass").click(function() {
    var ratevalue = $(".ratingsOnlyClass").data("rating");
    ...
    if (IsObjectRated) return;
    SubmitRatingOnly(ratevalue);
});



function SubmitRatingOnly(ratevalue) {
    try {
        // call web service
        var params = '{';
        params += '"guestId":"' + CurUserId + '"';
        ...
        params += ',"device":"' + Rating_Device + '"';
        params += ',"userIPAddress":"' + userIPAddress + '"';
        params += ',"userRating":"' + ratevalue + '"';
        params += ',"userComments":null';
        params += ',"logException":"true"';
        params += '}';

        jQuery.ajax({
            type: "POST",
            url: "/Platform/Controls/RatingsOnly/RatingsWebService.asmx/RateTopic",
            data: params,
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            async: false,
            success: function(msg) {
                IsObjectRated = true;
                $(".tbFont").innerHTML = RatedTooltipText;
                if (get_cookie(pageTag) != null) {
                    delete_cookie(pageTag);
                }
                var d = new Date();
                d.setFullYear(d.getFullYear() + 1)
                var CookieInfo = pageTag + "=" + escape("xxx") + ";expires=" + d.toGMTString() + " path=/;";
                document.cookie = CookieInfo;
                $(".ratingsOnlyClass").attr("isRated", "true");
            },
            error: function(response) {
...
// First events
function ShowShareThisChildPanels(strChildPanel) {
    if (jQuery('.ShareThis_ChildRootPanel').css('display') == 'none') {
        jQuery(".ShareThis_ChildRootPanel").slideDown();
        jQuery('.ShareThis_ChildRootPanel').css({
            left: (jQuery('.ShareThis_RootButton').attr('offsetLeft') + 6) + 'px',
            top: (jQuery('.ShareThis_RootButton').offset().top + 16) + 'px'
        });
    }
...
//	Method of removing the links panel
function CloseThisShareThisMenu() {
    jQuery(".ShareThis_ChildRootPanel").slideUp();
    return;
}

//	Method for "More sites" button
function ShowMoreSites(strThisId, strChildText) {
    jQuery(".tierTwoPanel").toggle("fast");
    jQuery("#" + strThisId + " > * ").toggle("fast");
    return;
}

jQuery('.ShareThis_RootButton a').bind("mouseover", function(e) {
    ShowShareThisChildPanels(this.id);
});
