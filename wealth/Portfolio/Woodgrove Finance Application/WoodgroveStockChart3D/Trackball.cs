    ...
    public class Trackball
    {
        ...
        private void MouseMoveHandler(object sender, System.Windows.Input.MouseEventArgs e)
        {
            ...
            if (el.IsMouseCaptured)
            {
                ...
                Quaternion qDelta = _rotationDelta;
                ...
                if (_rotating == true)
                {
                    ...
                    */
                    ...
                    if (len < 0.00001 || _scaling)
                        qDelta = new Quaternion(new Vector3D(0, 0, 1), 0);
                    else
                        qDelta = new Quaternion(axis, len);

                    q = qDelta * _rotation;

                    /*
                    if (Math.Abs(q.Angle) > 45)
                        return;
                    */
                    //string message = "Axis:[" + q.Axis.ToString() + "] Angle:[" + q.Angle.ToString() + "]";
                    //DebugHelp3D.Trace.Message(message);

                    _rotationDelta = qDelta;
                }
                else
                {
                    delta /= 20;
        ...
        private Point _pointRestriction;    // Restriction point of drag
